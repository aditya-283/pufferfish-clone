...
optimizer_vanilla = optim.SGD(
    net_vanilla.parameters(), lr=0.05, momentum=0.9, weight_decay=5e-4
)
...
def decompose_weights(model, low_rank_model, rank_factor):
    ...
    for item_index, (param_name, param) in enumerate(model.state_dict().items()):
        if (
            len(param.size()) == 4
            and item_index not in range(0, 2)
            and "shortcut" not in param_name
            and "downsample" not in param_name
        ):
            ...
            u_weight = u * torch.sqrt(
                s
            )  # alternative implementation: u_weight_alt = torch.mm(u, torch.diag(torch.sqrt(s)))
            v_weight = (
                torch.sqrt(s) * v
            )  # alternative implementation: v_weight_alt = torch.mm(torch.diag(torch.sqrt(s)), v.t())
            ...
            u_weight_sliced, v_weight_sliced = (
                u_weight[:, 0:sliced_rank],
                v_weight[:, 0:sliced_rank],
            )
            ...
            u_weight_sliced_shape, v_weight_sliced_shape = (
                u_weight_sliced.size(),
                v_weight_sliced.size(),
            )

            # print(u_weight_sliced_shape)
            model_weight_v = u_weight_sliced.view(
                u_weight_sliced_shape[0], u_weight_sliced_shape[1], 1, 1
            )

            model_weight_u = v_weight_sliced.t().view(
                v_weight_sliced_shape[1],
                param.size()[1],
                param.size()[2],
                param.size()[3],
            )
    ...
    model_counter = 0
    reload_state_dict = {}
    for item_index, (param_name, param) in enumerate(
        low_rank_model.state_dict().items()
    ):
        print(
            "#### {}, {}, recons agg: {}， param: {}".format(
                item_index,
                param_name,
    ...
    with torch.no_grad():
        ...
        logger.info(
            "Test @ Epoch: {}, Loss: {:.4f}, Acc: {:.2f}".format(
                epoch, test_loss / total, 100.0 * correct / total
            )
        )
...
TOTAL = 200
WARM_UP = 200
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer_vanilla, "min")
for epoch in range(start_epoch, start_epoch + TOTAL):
    ...
    if epoch in range(WARM_UP):
        ...
        transfer.fit_epoch(
            net_vanilla, transfer.trainloader, criterion, optimizer_vanilla
        )
        val_loss, _ = transfer.validate_epoch(
            net_vanilla, transfer.testloader, criterion
        )
        ...
        net = decompose_weights(
            model=net_vanilla, low_rank_model=net, rank_factor=RANK_FACTOR
        )
        ...
        transfer.fit_epoch(
            net, transfer.trainloader, criterion, optimizer
        )  # why different?
        transfer.validate_epoch(net, transfer.testloader, criterion)
