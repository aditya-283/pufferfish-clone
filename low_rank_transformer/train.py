...
def train(vanilla_model, training_data, validation_data, test_data, vanilla_optimizer, device, opt):
    ''' Start training '''
    ...
    valid_losses = []
    valid_bleu_scores = []
    est_rank_list = rank_estimation(-1, vanilla_model)
    ...
    for epoch_i in range(opt.epoch):
        ...
        if epoch_i in range(opt.fr_warmup_epoch):
            ...
            lowrank_model = AdaptTransformer(
                opt.src_vocab_size,
                opt.trg_vocab_size,
                ...
                est_ranks=est_rank_list).to(device)
...
def decompose_vanilla_model(vanilla_model, low_rank_model, est_rank_list):
    collected_weights = []
    rank_counter = 0
    for p_index, (name, param) in enumerate(vanilla_model.state_dict().items()):
        if len(param.size()) == 2 and p_index not in range(0, 14) and p_index not in range(76, 96) and p_index != 188:
            ...
            sliced_rank = est_rank_list[rank_counter]
            rank_counter += 1
...
def rank_estimation(epoch, model):
    est_ranks = []
    for p_index, (name, param) in enumerate(model.state_dict().items()):
        if len(param.size()) == 2 and p_index not in range(0, 14) and p_index not in range(76, 96) and p_index != 188:
            ...
            est_ranks.append(estimated_rank)
            logger.info("@@@ Epoch: {}, Layer index: {}, layer name: {}, Est Rank: {}, Ori rank: {}, layer norm: {}".format(
                                epoch, p_index, 
                                name, estimated_rank, ori_rank, torch.norm(param).data))
        else:
            pass
    return est_ranks
...
def main():
    ...
    train(vanilla_transformer, training_data, validation_data, test_data, vanilla_optimizer, device, opt)
