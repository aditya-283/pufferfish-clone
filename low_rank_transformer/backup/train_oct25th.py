...
def train(vanilla_model, lowrank_model, training_data, validation_data, test_data, vanilla_optimizer, device, opt):
    ''' Start training '''
    ...
    for epoch_i in range(opt.epoch):
        ...
        if opt.save_mode == 'all':
            ...
            if valid_loss <= min(valid_losses):
                ...
                print("##### On the test set of WMT 16' GE-to-En, Test Ppl.: {}, Test BLEU: {:.4f}".format(
                                                test_ppl, test_bleu))
...
def main():
    ...
    vanilla_optimizer = ScheduledOptim(
        optim.AdamW(vanilla_transformer.parameters(), betas=(0.9, 0.98), eps=1e-09, weight_decay=5e-4),
        opt.lr_mul, opt.d_model, opt.n_warmup_steps)
    #lowrank_optimizer = ScheduledOptim(
    #    optim.Adam(lowrank_transformer.parameters(), betas=(0.9, 0.98), eps=1e-09),
    #    2.0, opt.d_model, opt.n_warmup_steps)

    train(vanilla_transformer, lowrank_transformer, training_data, validation_data, test_data, vanilla_optimizer, device, opt)
